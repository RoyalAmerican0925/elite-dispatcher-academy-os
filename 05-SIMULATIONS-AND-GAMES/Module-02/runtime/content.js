...
export const COMPETENCY_LABELS = {
  ...
  B: "Tracing the source/control of the opportunity",
  ...
  G: "Recognizing insufficient facts and selecting a defensible operational action",
  H: "Applying STOP / IDENTIFY / TRACE / CHECK / CLASSIFY / ACT / DOCUMENT / ESCALATE",
};
...
export const DECISIONS = {
  ...
  "SIM02-D09": {
    ...
    choices: [
      { key: "A", text: "Consistent with Carrier-Side Agency on the supplied facts — the stated non-overlap removes a competing-carrier choice for this specific comparison" },
      { key: "B", text: "Significant Brokerage Risk — representing two carriers is inherently risky" },
      { key: "C", text: "More Facts Required solely because compensation is unknown" },
    ],
    correctKey: "A",
    correctFeedback: "On the supplied facts, the carriers have genuinely non-overlapping equipment and territory, so this comparison does not present a discretionary choice between them for the same freight. Non-overlap is relevant evidence; it is not an automatic legal safe harbor.",
    remediation: "Contrast this with the overlapping roster in the prior decision. Here the supplied facts remove the competing-carrier choice for this opportunity, but non-overlap does not create a universal safe harbor and the complete relationship and conduct still matter.",
  },
  "SIM02-D10": {
    id: "SIM02-D10",
    stageId: "SIM02-S5",
    ...
    correctKey: "A",
    correctFeedback: "Returning a declined opportunity to its source is materially different from taking control of it and reallocating it yourself. Returning it is the low-risk path on these facts; reallocating it is where the allocation concern begins.",
    remediation: "Declining and returning an opportunity to its source is not the same as taking control of it and choosing a different carrier yourself. The latter introduces an allocation-of-traffic concern; do not treat return-to-source as authority to arrange replacement transportation.",
    escalateAfterAttempts: 2,
  },
  "SIM02-D11": {
    id: "SIM02-D11",
  ...
  "SIM02-D14": {
    ...
    choices: [
      ...
      { key: "B", text: "Assume it's risky because the missing facts probably indicate brokerage" },
      { key: "C", text: "STOP, identify and trace the missing facts, CHECK/VERIFY them, preserve an insufficient-information classification while unresolved, HOLD the action, DOCUMENT the gap, and ESCALATE if qualified review is needed" },
      { key: "D", text: "Guess based on which classification seems more common in this simulation" },
    ],
    correctKey: "C",
    correctFeedback: "With controlling facts missing, use the full model: STOP → IDENTIFY → TRACE → CHECK → CLASSIFY → ACT → DOCUMENT → ESCALATE. Trace whether this load was offered for one carrier and whether any allocation choice existed; classify as insufficient verified information while the gap remains; HOLD/VERIFY instead of guessing; document the unresolved facts and escalate when qualified review is required.",
    remediation: "An ongoing relationship does not resolve this specific load's facts. TRACE the opportunity, CHECK the missing facts, preserve the insufficient-information classification, ACT by holding/verifying rather than guessing, DOCUMENT what remains unresolved, and ESCALATE if the issue cannot be resolved through ordinary verification.",
  },
  "SIM02-D15": {
    id: "SIM02-D15",
    stageId: "SIM02-S7",
    type: "sequencing",
    competency: "H",
    prompt: "Put the complete Module 02 operational response steps in order for this situation.",
    items: [
      { key: "identify", text: "IDENTIFY the represented principal and exactly which material facts are missing" },
      { key: "stop", text: "STOP — do not proceed on an incomplete fact pattern" },
      { key: "trace", text: "TRACE the freight opportunity to its source and determine whether a specific carrier was already tied to it" },
      { key: "check", text: "CHECK/VERIFY the missing facts and supporting evidence" },
      { key: "classify", text: "CLASSIFY from verified facts; use insufficient verified information while a material gap remains" },
      { key: "act", text: "ACT — PROCEED, HOLD, VERIFY, ESCALATE, or DO NOT PROCEED AS PROPOSED based on the verified facts" },
      { key: "document", text: "DOCUMENT the known facts, unknowns, reasoning, action, and follow-up owner" },
      { key: "escalate", text: "ESCALATE with a specific professional handoff when qualified review is required" },
    ],
    correctOrder: ["stop", "identify", "trace", "check", "classify", "act", "document", "escalate"],
    correctFeedback: "The full operational sequence is STOP → IDENTIFY → TRACE → CHECK → CLASSIFY → ACT → DOCUMENT → ESCALATE. Each step produces evidence for the next; escalation is a professional handoff when the unresolved issue requires qualified review, not a substitute for doing the earlier fact work.",
    remediation: "Do not collapse the workflow into a four-step shortcut. Stop first, identify the principal and gap, trace the opportunity, check/verify evidence, classify from verified facts, select the operational action, document the decision record, and escalate with a specific question when required.",
  },
};
