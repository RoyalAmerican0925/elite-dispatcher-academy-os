// SIM-002 browser application layer.
// Isolated copy adapted from SIM-001's app.js — no shared import — same reasoning as
...
const STORAGE_KEY = "sim002-state-v1";
...
function loadState() {
  try {
    ...
    return null; // corrupt JSON — fall through to a fresh state
  }
}
...
function renderStartScreen() {
  return el("div", { class: "start-screen" }, [
    el("h1", {}, "Module 02 \u2014 Agency Boundary Desk"),
    el(
      "p",
      {},
      "A seven-stage dispatch-desk simulation reinforcing Module 02: recognizing the boundary between carrier-side dispatching and conduct that presents brokerage risk \u2014 based on actual conduct, not labels."
    ),
    el(
      "button",
      {
...
function renderCompletionScreen() {
  ...
  const container = el("div", { class: "completion-screen" }, [
    ...
    el(
      ...
      isReview
        ...
        : "You've worked through the agency-boundary judgment involved in a full week of dispatch decisions."
    ),
  ]);
  ...
  container.appendChild(
    el(
      "p",
      { class: "next-step" },
      "Next step: complete the Module 02 Knowledge Check, Exam, and Practical Assessment if you haven't already \u2014 this simulation reinforces that material, it doesn't replace it."
    )
  );
...
function renderCurrent() {
  root.innerHTML = "";
  root.appendChild(
    el("header", { class: "app-header" }, [el("div", { class: "app-title" }, "Elite Dispatcher Academy \u2014 SIM-002"), renderResetControl()])
  );
