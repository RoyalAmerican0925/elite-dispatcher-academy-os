// SIM-003 browser application layer.
// Isolated copy adapted from SIM-002's app.js — no shared import — same reasoning as
// content.js/logic.js. All correctness/scoring logic lives in logic.js and is imported.
...
const STORAGE_KEY = "sim003-state-v1";
...
function saveState(state) {
  try {
    ...
    // Storage can fail (private browsing, quota, etc.) — not fatal for the current session.
  }
}
...
function renderChoice(decision) {
  ...
  for (const choice of decision.choices) {
    ...
    fieldset.appendChild(
      el("label", { for: inputId, class: "choice-option" }, [
        input,
...
function renderMultiselect(decision) {
  if (!Array.isArray(pendingResponse)) pendingResponse = [];
  const fieldset = el("fieldset", { class: "choice-group" }, [
    el("legend", { class: "sr-only" }, `${decision.prompt} (select all that apply)`),
  ]);
  for (const opt of decision.options) {
    const inputId = `multi-${decision.id}-${opt.key}`;
    const input = el("input", {
      type: "checkbox",
      id: inputId,
      value: opt.key,
      onchange: (e) => {
        const set = new Set(pendingResponse);
        if (e.target.checked) set.add(opt.key);
        else set.delete(opt.key);
        pendingResponse = Array.from(set);
      },
    });
    if (pendingResponse.includes(opt.key)) input.checked = true;
    fieldset.appendChild(
      el("label", { for: inputId, class: "choice-option" }, [input, el("span", { class: "choice-text" }, opt.text)])
    );
  }
  return fieldset;
}
...
function renderResponseControl(decision) {
  ...
  if (decision.type === "multiselect") return renderMultiselect(decision);
...
function isResponseComplete(decision) {
  ...
  if (decision.type === "multiselect") return Array.isArray(pendingResponse) && pendingResponse.length > 0;
...
function renderFeedback(decisionId, correct) {
  ...
  if (record.instructorReviewRequired) {
    panel.appendChild(
      el(
        "p",
        { class: "feedback-escalation" },
        "This item has been flagged for instructor review after repeated attempts. You'll move on, and an instructor will follow up on this specific decision."
      )
    );
  }
...
function renderStartScreen() {
  return el("div", { class: "start-screen" }, [
    el("h1", {}, "Module 03 \u2014 Load Compatibility Desk"),
    el(
      "p",
      {},
      "A seven-stage dispatch-desk simulation reinforcing Module 03: evaluating whether a specific load can safely and operationally move with a specific carrier, driver, and equipment combination \u2014 checking equipment, commodity, temperature, dimensions, weight/axles, and driver qualifications together, never from a single fact alone."
    ),
    el(
      "button",
      {
...
function renderCompletionScreen() {
  ...
  const container = el("div", { class: "completion-screen" }, [
    ...
    el(
      ...
      isReview
        ...
        : "You've worked through a full week at the load compatibility desk \u2014 equipment, commodity, temperature, dimensions, weight and axles, driver qualifications, and missing-information discipline."
    ),
  ]);

  container.appendChild(
    el("div", { class: "completion-stats" }, [
  ...
  for (const [code, label] of Object.entries(COMPETENCY_LABELS)) {
    ...
    competencyList.appendChild(
      el("div", { class: "competency-row" }, [el("span", { class: "competency-label" }, label), el("span", { class: `competency-status ${statusClass}` }, status)])
    );
  }
  ...
  container.appendChild(
    el(
      "p",
      { class: "next-step" },
      "Next step: complete the Module 03 Knowledge Check, Exam, and Practical Assessment if you haven't already \u2014 this simulation reinforces that material, it doesn't replace it."
    )
  );
...
function renderCurrent() {
  root.innerHTML = "";
  root.appendChild(
    el("header", { class: "app-header" }, [el("div", { class: "app-title" }, "Elite Dispatcher Academy \u2014 SIM-003"), renderResetControl()])
  );
